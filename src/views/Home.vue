...
:root {
  ...
  --text-light: #fff8e1; /* Màu chữ sáng */
}
...
.wuxia-dashboard {
  ...
  /* Đổi màu chữ mặc định sang màu sáng để nổi trên nền gỗ tối */
  color: var(--text-light);
  font-family: "Noto Serif TC", serif; /* Font nét dày hơn */
  position: relative;
  overflow: hidden;
}
...
.ink-bg-layer {
  ...
  inset: 0;
  z-index: 0;
  background-color: #3e2723; /* Nền tối an toàn */
}
...
.mountain-bg {
  ...
  filter: sepia(30%) brightness(0.7) contrast(1.1); /* Làm tối ảnh nền để chữ nổi hơn */
  opacity: 0.9;
}
...
.fog-anim {
  ...
  background: linear-gradient(
    to top,
    rgba(62, 39, 35, 0.9) 0%,
    /* Fog màu nâu gỗ tối */ transparent 60%
  );
}
...
.command-header {
  ...
  padding-bottom: 20px;
  border-bottom: 3px double rgba(255, 255, 255, 0.3); /* Viền sáng */
}
...
.welcome-text {
  ...
  text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.8); /* Shadow đậm để tách nền */
}
...
/* Base Card Style (Thẻ gỗ/giấy) */
.cmd-card {
  position: relative;
  text-decoration: none;
  background: var(--paper-bg); /* Nền giấy sáng */
...
/* Chỉnh màu chữ trong thẻ (Card) thành màu đậm vì nền thẻ là màu sáng */
h3 {
  margin: 0;
  font-family: "Noto Serif TC";
  font-size: 1.3rem;
  color: #212121; /* Đen mực */
  font-weight: 900;
}
p {
  margin: 5px 0 0;
  color: #4e342e; /* Nâu đậm thay vì nâu nhạt */
  font-size: 0.95rem;
  font-weight: 600;
}

/* --- PLAY CARD (THẺ NHẬP THẾ - NỀN TỐI) --- */
.play-card {
  grid-column: span 2;
  grid-row: span 2;
...
/* Vì thẻ này nền tối, chữ phải sáng */
.play-card h2 {
  font-family: "Noto Serif TC";
  font-size: 4rem;
  margin: 0;
...
/* Admin card nền tối nên chữ phải sáng */
.admin-card h3 {
  color: #fff;
}
.admin-card p {
...
.ticker-track {
  ...
  align-items: center;
  overflow: hidden;
  background: #fff8e1; /* Nền giấy sáng */
}
