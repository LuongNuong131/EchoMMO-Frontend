        ...
        <div class="log-panel">
          ...
          <div class="log-content custom-scroll">
            <div v-for="(log, index) in logs" :key="index" class="log-line">
              <span class="log-time">[{{ log.time }}]</span><span class="log-msg" v-html="log.msg"></span>
            </div>
          </div>
        </div>
      </div>
...
// [FIX] Import getCurrentSkin thay vì getCharImage
...
// [FIX] Logic lấy ảnh dựa trên Skin ID (avatarUrl) của User
const imgPlayer = computed(() => {
  // authStore.user.avatarUrl bây giờ chứa ID skin (vd: "skin_yasou")
  const skin = getCurrentSkin(authStore.user?.avatarUrl);

  if (isMoving.value) {
    return skin.sprites.run;
  }
...
const flee = () => { isEncounter.value = false; showTarget.value = false; addLog("<span style='color:#a5d6a7'>Đã chạy thoát.</span>"); };
...
const startExploration = () => {
  ...
  showTarget.value = false;
  isEncounter.value = false;
  countdown.value = 2; // Thời gian chạy animation
  startMovingJS();
  addLog("Bắt đầu hành tẩu...");
  const timer = setInterval(async () => {
    countdown.value--;
...
// [LOGIC CHÍNH] Xử lý kết quả hành tẩu
const handleResult = async () => {
  clearInterval(moveInterval);
  isMoving.value = false;

  // 1. Tỉ lệ 20% gặp Tài Nguyên (Gathering Event)
  ...
  if (eventChance < 20) {
    ...
    // Chuyển hướng sang trang Gathering
    setTimeout(() => {
      router.push('/gathering');
    }, 1000);
    return; // Dừng lại, không gọi API explore phía dưới
  }

  // 2. Tỉ lệ 80% còn lại: Gọi API Explore (Gặp Quái hoặc Vàng)
  try {
    const res = await charStore.explore(); // Backend sẽ cộng Vàng và Exp tại đây

    if (res.type === 'GOLD') {
      showTarget.value = true;
      targetName.value = "Túi Vàng";
      targetImage.value = getItemImage('GOLD');
      // Hiển thị Log: Vàng và EXP đã được cộng tích
      addLog(`<span style="color:#ffd700; font-weight:bold;">${res.message}</span>`);
    } else if (res.type === 'ENEMY') {
      isEncounter.value = true;
      showTarget.value = true;
...
<!-- 6:04 -->
 // File: luongnuong131/echommo-frontend/EchoMMO-Frontend-94f2ce592496ad5ec430f3836e4f93775cd71963/src/views/Explore.vue
...
.right-zone {
  ...
  overflow: hidden;
  height: 100%;
  gap: 15px; /* Thêm khoảng cách giữa các panel */
}

.log-panel {
  flex: 0 0 40%; /* Giảm chiều cao log panel (40% tổng chiều cao của right-zone) */
...
.quest-panel-wrapper {
  ...
  border-radius: 8px;
  overflow: hidden;
  padding: 10px; /* Thêm padding cho QuestPanel */
  display: flex; /* Để QuestPanel bên trong chiếm hết 100% */
}
...
@media (max-width: 900px) {
  ...
  .right-zone {
    height: 350px; /* Giữ fixed height trên mobile cho 2 panel */
    min-height: 300px;
  }
}
</style>