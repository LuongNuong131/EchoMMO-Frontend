    ...
    <div class="admin-wrapper custom-scroll">
      <div class="admin-header">
        ...
        <div class="tabs-scroll-wrapper">
          <div class="wuxia-tabs">
            <button :class="{ active: activeTab === 'stats' }" @click="switchTab('stats')" class="tab-btn">
              <i class="fas fa-chart-line"></i> THỐNG KÊ
            </button>
            <div class="tab-divider"></div>
            <button :class="{ active: activeTab === 'users' }" @click="switchTab('users')" class="tab-btn">
              <i class="fas fa-users"></i> NHÂN SĨ
            </button>
            <div class="tab-divider"></div>
            <button :class="{ active: activeTab === 'items' }" @click="switchTab('items')" class="tab-btn">
              <i class="fas fa-khanda"></i> KHO TÀNG
            </button>
            <div class="tab-divider"></div>
            <button :class="{ active: activeTab === 'grant' }" @click="switchTab('grant')" class="tab-btn">
              <i class="fas fa-gift"></i> BAN THƯỞNG
            </button>
            <div class="tab-divider"></div>
            <button :class="{ active: activeTab === 'notify' }" @click="switchTab('notify')" class="tab-btn">
      ...
      <div v-if="activeTab === 'stats'" class="stats-grid">
        ...
        <div class="stat-card gold-card">
          ...
          <div class="stat-info">
            <h3>NGÂN LƯỢNG</h3>
            <p class="num gold-text">{{ formatNumber(adminStore.stats.totalGold || 0) }}</p>
          </div>
            ...
            <tbody>
              <tr v-for="u in adminStore.users" :key="u.userId">
                ...
                <td class="action-cell">
                  <button v-if="u.isActive" @click="openBanModal(u)" class="btn-icon red" title="Giam cầm">
                    <i class="fas fa-gavel"></i>
                  </button>
                  <button v-else @click="unbanUser(u.userId)" class="btn-icon green" title="Phóng thích">
                    <i class="fas fa-lock-open"></i>
                  </button>
                  <button @click="adminStore.deleteUser(u.userId)" class="btn-icon gray" title="Xóa bỏ">
      ...
      <div v-if="activeTab === 'items'" class="content-panel">
        <div class="panel-header flex-between">
          ...
          <button @click="showCreateItem = !showCreateItem" class="btn-wood small">
        ...
        <transition name="slide-down">
          <div v-if="showCreateItem" class="create-form-box">
            <form @submit.prevent="createItem" class="ancient-form">
              <div class="form-grid">
                <input v-model="itemForm.name" placeholder="Tên vật phẩm..." class="dark-input" required />
                <input v-model="itemForm.description" placeholder="Mô tả..." class="dark-input" />
                ...
                <input v-model.number="itemForm.basePrice" type="number" placeholder="Giá bán" class="dark-input" />
              </div>
              
              <div class="form-row mt-10">
                <input v-model="itemForm.imageUrl" placeholder="URL Hình ảnh..." class="dark-input full-width" />
              </div>
          ...
          <table class="dark-table">
            ...
            <tbody>
              <tr v-for="i in adminStore.items" :key="i.itemId">
                ...
                <td>
                  <button @click="adminStore.deleteItem(i.itemId)" class="btn-icon red">
      ...
      <div v-if="activeTab === 'grant'" class="content-panel">
        ...
        <div class="grant-grid">
          <div class="grant-box">
            <h4 class="sub-title"><i class="fas fa-coins"></i> CẤP NGÂN LƯỢNG</h4>
            <form @submit.prevent="handleGrantGold" class="ancient-form">
              <input v-model="grantGoldForm.username" placeholder="Danh tính (username)" class="dark-input mb-10" required />
              <input v-model.number="grantGoldForm.amount" type="number" placeholder="Số lượng" class="dark-input mb-10" min="1" required />
              <button type="submit" class="btn-wood gold full-width">BAN THƯỞNG</button>
            </form>
          </div>
          ...
          <div class="grant-box">
            ...
            <form @submit.prevent="handleGrantItem" class="ancient-form">
              <input v-model="grantItemForm.username" placeholder="Danh tính (username)" class="dark-input mb-10" required />
              <select v-model.number="grantItemForm.itemId" class="dark-input mb-10" required>
                <option :value="0" disabled>Chọn vật phẩm...</option>
                <option v-for="item in itemOptions" :key="item.id" :value="item.id">
                  #{{ item.id }} - {{ item.name }}
                </option>
              </select>
              <input v-model.number="grantItemForm.quantity" type="number" placeholder="Số lượng" class="dark-input mb-10" min="1" required />
              <button type="submit" class="btn-wood primary full-width">BAN THƯỞNG</button>
            </form>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTab === 'notify'" class="content-panel">
        ...
        <div class="notify-box">
          <form @submit.prevent="handleSendNotification" class="ancient-form">
            <div class="form-grid">
              <input v-model="notificationForm.title" placeholder="Tiêu đề..." class="dark-input" required />
            ...
            <input v-model="notificationForm.recipientUsername" placeholder="Người nhận (Để trống = Toàn server)" class="dark-input mt-10" />
            <textarea v-model="notificationForm.message" placeholder="Nội dung cáo thị..." class="dark-input text-area mt-10" rows="5" required></textarea>
    ...
    <div v-if="showBanModal" class="modal-overlay" @click.self="showBanModal = false">
      <div class="dark-modal">
        ...
        <div class="modal-body">
          <p class="target-user">Tội nhân: <span>{{ selectedUser?.username }}</span></p>
          <textarea v-model="banReason" placeholder="Lý do trừng phạt..." class="dark-input text-area" rows="3"></textarea>
...
const itemForm = reactive({
    name: "", description: "", type: "WEAPON", rarity: "C",
    basePrice: 100, imageUrl: "", isSystemItem: false,
});
...
const handleGrantGold = async () => {
    ...
    await adminStore.grantGold({ username: grantGoldForm.username, amount: grantGoldForm.amount });
...
const handleGrantItem = async () => {
    ...
    await adminStore.grantItem({ username: grantItemForm.username, itemId: grantItemForm.itemId, quantity: grantItemForm.quantity });
    grantItemForm.itemId = 0; grantItemForm.quantity = 1;
};
...
const handleSendNotification = async () => {
    ...
    notificationForm.title = ""; notificationForm.message = ""; notificationForm.recipientUsername = "";
};
...
const confirmBan = async () => {
    ...
    alert("Đã thi hành án!"); showBanModal.value = false;
};
const unbanUser = async (id) => {
    if (confirm("Phóng thích?")) { await adminStore.unbanUser(id); alert("Đã ân xá!"); }
};

onMounted(() => { adminStore.fetchStats(); adminStore.fetchItems(); });
</script>
...
.stat-info h3 { margin: 0; font-size: 0.9rem; color: #aaa; letter-spacing: 1px; }
.stat-info .num { margin: 5px 0 0; font-size: 2rem; font-weight: bold; color: #fff; }
.gold-text { color: var(--gold) !important; text-shadow: 0 0 5px rgba(255, 215, 0, 0.5); }
...
.dim-text { color: #777; font-size: 0.9rem; }
...
.status-tag.active { color: #4caf50; border: 1px solid #4caf50; }
.status-tag.banned { color: #f44336; border: 1px solid #f44336; }
...
.btn-icon.red { color: #e57373; }
.btn-icon.green { color: #81c784; }
.btn-icon.gray { color: #9e9e9e; }
...
.check-label { display: flex; align-items: center; gap: 5px; cursor: pointer; color: #aaa; }

.btn-wood {
    background: var(--wood-light); border: 1px solid var(--gold); color: var(--gold);
    padding: 10px 20px; font-weight: bold; cursor: pointer; font-family: inherit;
    transition: 0.2s;
}
.btn-wood:hover { background: var(--gold); color: var(--wood-dark); }
.btn-wood.primary { background: var(--red-seal); color: #fff; border: none; }
.btn-wood.primary:hover { background: #d32f2f; }
.btn-wood.small { padding: 5px 15px; font-size: 0.9rem; }
.btn-wood.gold { background: var(--gold); color: var(--wood-dark); }
.btn-wood.red { background: var(--red-seal); color: #fff; border: none; }

/* GRANT GRID */
.grant-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 20px; }
.grant-box {
    background: rgba(0,0,0,0.2); padding: 20px;
    border: 1px solid #444;
}
.sub-title { margin: 0 0 15px 0; color: #ccc; border-bottom: 1px solid #444; padding-bottom: 5px; }

/* NOTIFY */
.notify-box { max-width: 600px; margin: 0 auto; }
...
.dark-modal {
    width: 400px; background: var(--wood-dark);
    border: 2px solid var(--red-seal);
    box-shadow: 0 0 30px rgba(0,0,0,0.9);
}
.modal-header {
    background: var(--red-seal); color: #fff; padding: 10px;
    text-align: center; font-weight: bold;
}
.modal-body { padding: 20px; text-align: center; }
.target-user { font-size: 1.1rem; margin-bottom: 15px; }
.target-user span { color: var(--red-seal); font-weight: bold; }
...
.rarity-S { color: var(--gold); font-weight: bold; }
